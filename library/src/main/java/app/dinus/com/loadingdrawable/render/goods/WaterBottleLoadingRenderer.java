...
public class WaterBottleLoadingRenderer extends LoadingRenderer {
    ...
    @Override public void draw(Canvas canvas, Rect bounds) {
        ...
        for (WaterDropHolder waterDropHolder : mWaterDropHolders) {
            if (waterDropHolder.mNeedDraw) {
                canvas.drawCircle(waterDropHolder.mInitX, waterDropHolder.mCurrentY,
                                  waterDropHolder.mRadius, mPaint);
            }
        }
    ...
    @Override public void computeRender(float renderProgress) {
        ...
        mBottleBounds.set(arcBounds.centerX() - mBottleWidth / 2.0f,
                          arcBounds.centerY() - mBottleHeight / 2.0f,
                          arcBounds.centerX() + mBottleWidth / 2.0f,
                          arcBounds.centerY() + mBottleHeight / 2.0f);
        //compute pipe body bounds
        mWaterBounds.set(mBottleBounds.left + getStrokeWidth() * 1.5f,
                         mBottleBounds.top + mWaterLowestPointToBottleneckDistance,
                         mBottleBounds.right - getStrokeWidth() * 1.5f,
                         mBottleBounds.bottom - getStrokeWidth() * 1.5f);
        ...
        for (WaterDropHolder waterDropHolder : mWaterDropHolders) {
            if (waterDropHolder.mDelayDuration < renderProgress
                ...
                float riseProgress =
                        (renderProgress - waterDropHolder.mDelayDuration) / waterDropHolder.mDuration;
                riseProgress =
                        riseProgress < 0.5f ? riseProgress * 2.0f : 1.0f - (riseProgress - 0.5f) * 2.0f;
                waterDropHolder.mCurrentY = waterDropHolder.mInitY
                        - MATERIAL_INTERPOLATOR.getInterpolation(riseProgress)
                        * waterDropHolder.mRiseHeight;
    ...
    private Path createBottlePath(RectF bottleRect) {
        ...
        path.quadTo(bottleRect.centerX() - bottleneckDecorationWidth * 0.5f - bottleneckWidth * 0.15f,
                    bottleRect.top + bottleneckDecorationHeight * 0.5f,
                    bottleRect.centerX() - bottleneckWidth * 0.5f,
                    bottleRect.top + bottleneckDecorationHeight);
        ...
        path.lineTo(bottleRect.centerX() + bottleneckWidth * 0.5f,
                    bottleRect.top + bottleneckDecorationHeight);
        path.quadTo(bottleRect.centerX() + bottleneckDecorationWidth * 0.5f + bottleneckWidth * 0.15f,
                    bottleRect.top + bottleneckDecorationHeight * 0.5f,
    ...
    private Path createWaterPath(RectF waterRect, float progress) {
        ...
        path.cubicTo(waterRect.left + waterRect.width() * 0.80f - cubicXChangeSize,
                     waterRect.top - waterRect.height() * 1.2f + cubicYChangeSize,
                     waterRect.left + waterRect.width() * 0.55f - cubicXChangeSize,
                     waterRect.top - cubicYChangeSize, waterRect.left,
                     waterRect.top - getStrokeWidth() / 2.0f);
    ...
    private void initWaterDropHolders(RectF bottleRect, RectF waterRect) {
        ...
        for (int i = 0; i < DEFAULT_WATER_DROP_COUNT; i++) {
            ...
            waterDropHolder.mInitX =
                    bottleRect.left + twoSidesInterval + mRandom.nextFloat() * currentXRandomRange;
            waterDropHolder.mInitY = lowestWaterPointY + waterDropHolder.mRadius / 2.0f;
            waterDropHolder.mRiseHeight = getMaxRiseHeight(bottleRadius, waterDropHolder.mRadius,
                                                           waterDropHolder.mInitX - bottleRect.left) * (0.2f
                    + 0.8f * mRandom.nextFloat());
            waterDropHolder.mDelayDuration =
                    atLeastDelayDuration + mRandom.nextFloat() * delayDurationRange;
    ...
    private float getMaxRiseHeight(float bottleRadius, float waterDropRadius, float currentX) {
        ...
        return (float) (Math.sqrt(Math.pow(bottleRadius, 2.0f) - Math.pow(coordinateX, 2.0f))
                - waterDropRadius);
    }
